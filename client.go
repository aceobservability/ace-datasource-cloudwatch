...
func (c *Client) awsConfig(ctx context.Context) (aws.Config, error) {
	...
	if strings.TrimSpace(c.parsed.AccessKeyID) != "" || strings.TrimSpace(c.parsed.SecretAccessKey) != "" {
		if strings.TrimSpace(c.parsed.AccessKeyID) == "" || strings.TrimSpace(c.parsed.SecretAccessKey) == "" {
		...
		loadOptions = append(loadOptions, awsconfig.WithCredentialsProvider(credentials.NewStaticCredentialsProvider(
			c.parsed.AccessKeyID,
			c.parsed.SecretAccessKey,
...
func parseCloudWatchLogResults(rows [][]cloudwatchlogstypes.ResultField) []datasource.LogEntry {
	entries := make([]datasource.LogEntry, 0, len(rows))
